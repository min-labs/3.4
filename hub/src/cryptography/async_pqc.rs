...
//   This module eliminates that failure domain entirely.
...
/// # Safety
/// - `payload_arena` must point to a valid array of `max_peers` elements of
///   `[u8; MAX_HS_PAYLOAD_SIZE]`, readable from this thread.
/// - `hs_state_arena` must point to a valid array of `max_peers` elements of
///   `FlatHubHandshakeState`, exclusively writable by this thread.
/// - Both pointers must remain valid for the lifetime of this thread.
pub unsafe fn pqc_worker_thread(
    core_id: usize,
    ...
    loop {
        ...
        for req in req_buf.iter().take(n) {
