...
impl SimdGfMultiplier {
    ...
    /// # Safety
    /// - `dst` and `src` must point to valid buffers of at least `len` bytes.
    ...
    pub unsafe fn mac_region(&self, dst: *mut u8, src: *const u8, len: usize) {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            ...
            while offset + 64 <= len {
                ...
                // AVX-512 byte shift right by 4 using srli_epi16 and masking
...
impl EncoderWindow {
    ...
    /// Heap-allocate a window (11KB+ — bypass stack).
...
impl Default for SwRlncDecoder {
    fn default() -> Self { Self::new() }
}
...
impl SwRlncDecoder {
    ...
    pub fn ingest(&mut self, abs_seq: u32, is_parity: bool, seed: u8, data: &[u8]) -> bool {
        ...
        // BUG FIX: iterate ALL columns (0..k, j != i), not just (i+1)..k.
        // R→L pivot means parity-derived rows have non-zero entries LEFT of their
        // pivot column (at unfilled/dropped positions). The original (i+1)..k loop
        // misses those, producing corrupted payloads during recovery.
    ...
    pub fn ingest_parity(&mut self, seed: u8, k: usize, data: &[u8]) -> bool {
        ...
        // Forward elimination — full-row scan (same R→L-safe fix)
        self.reduce_row(row_eq, pivot_data, len)
    }
...
mod tests {
    ...
    // ============================================================================
    // AUDIT VERIFICATION TESTS — verify/refute uncertain findings
    // ============================================================================

    /// AUDIT GAP #3: deliver_next = fec_data_count skips recovered frames.
    /// Simulates: receive data 0,2,3 (skip 1), set deliver_next=3, parity recovers 1.
    /// Expected: slot 1 is decoded but deliver() starts at 3 → recovered frame LOST.
    #[test]
    fn audit_deliver_next_hack_loses_recovered_frames() {
        let payloads: Vec<[u8; USO_MTU]> = (0..4).map(|i| {
            let mut p = [0u8; USO_MTU];
            for (j, item) in p.iter_mut().enumerate().take(USO_MTU) { *item = ((i * 37 + j) & 0xFF) as u8; }
            p
        ...
        let mut window = EncoderWindow::new();
        for payload in payloads.iter().take(4) { unsafe { window.push(payload.as_ptr(), USO_MTU); } }
        let seed = 42u8;
        let mut parity = [0u8; USO_MTU];
        encode_parity_payload(&mut parity, &window, 4, seed);

        let mut decoder = SwRlncDecoder::new_boxed();
        // Feed frames 0, 2, 3 — frame 1 lost
        decoder.ingest(0, false, 0, &payloads[0]);
        decoder.ingest(2, false, 0, &payloads[2]);
        decoder.ingest(3, false, 0, &payloads[3]);

        // Simulate the node's deliver_next hack
        decoder.deliver_next = 3; // fec_data_count = 3

        // Parity recovers slot 1
        decoder.ingest_parity(seed, 4, &parity);
        assert!(decoder.is_decoded(1), "Parity must recover slot 1");

        // deliver() starts at slot 3 — slot 1 is skipped!
        let mut delivered = Vec::new();
        decoder.deliver(|p| delivered.push(p[0]));

        // CONFIRM BUG: recovered frame at slot 1 never delivered
        let slot1_byte = payloads[1][0];
        let slot1_delivered = delivered.contains(&slot1_byte);
        assert!(!slot1_delivered,
            "BUG CONFIRMED: deliver_next=3 skips recovered slot 1. \
             Delivered: {:?}, expected slot 1 byte: {}", delivered, slot1_byte);
    }

    /// AUDIT GAP #3b: Large seq_id overflows MAX_K window, frame discarded.
    /// Simulates: zero-reset decoder, ingest abs_seq=1000 → relative_idx=1000 > MAX_K=64.
    #[test]
    fn audit_large_seq_overflow_resets_decoder() {
        let mut decoder = SwRlncDecoder::new_boxed();
        assert_eq!(decoder.base_seq, 0, "Fresh decoder base_seq must be 0");

        let payload = [0xABu8; USO_MTU];

        // Ingest seq=1000: relative_idx = 1000-0 = 1000 >= MAX_K=64
        // → advance_window(1000-64+1 = 937) → returns false
        let gained = decoder.ingest(1000, false, 0, &payload);
        assert!(!gained, "BUG CONFIRMED: seq=1000 with base_seq=0 triggers advance_window, frame lost");
        assert_eq!(decoder.rank(), 0, "Decoder must have rank 0 after overflow advance");
    }

    /// AUDIT GAP #3c: Correct approach — leave deliver_next at 0, call deliver twice.
    /// Before parity: delivers slot 0, stops at gap (slot 1).
    /// After parity: delivers slots 1, 2, 3. Total = 4.
    #[test]
    fn audit_correct_two_phase_deliver() {
        let payloads: Vec<[u8; USO_MTU]> = (0..4).map(|i| {
            let mut p = [0u8; USO_MTU];
            p[0] = i as u8;
            for (j, item) in p.iter_mut().enumerate().take(USO_MTU).skip(1) { *item = ((i * 37 + j) & 0xFF) as u8; }
            p
        }).collect();
        ...
        // Phase 1: deliver before parity
        let mut phase1 = Vec::new();
        decoder.deliver(|p| phase1.push(p[0]));
        assert_eq!(phase1, vec![0], "Phase 1 should deliver slot 0, stop at gap (slot 1)");

        // Parity recovers slot 1
        decoder.ingest_parity(seed, 4, &parity);
        assert!(decoder.is_decoded(1), "Parity must fill slot 1");

        // Phase 2: deliver after parity
        let mut phase2 = Vec::new();
        decoder.deliver(|p| phase2.push(p[0]));
        assert_eq!(phase2, vec![1, 2, 3], "Phase 2 should deliver slots 1, 2, 3 contiguously");
    }
}
