...
pub const TX_PARITY_ENTRIES: u32 = 128;
// EXACT GEOMETRY: Data allocation must encompass both rings and parity TX bufs.
pub const TOTAL_BIDS: u32 = UDP_RING_ENTRIES + TUN_RX_ENTRIES + TX_PARITY_ENTRIES;
...
pub const TAG_UDP_SEND_PARITY: u64 = 6;
...
impl UringReactor {
    pub fn new(sock_fd: RawFd, sq_thread_cpu: u32) -> Self {
        ...
        let mut free_tx_bids = Vec::with_capacity(TX_PARITY_ENTRIES as usize);
        for i in 0..TX_PARITY_ENTRIES {
            free_tx_bids.push((UDP_RING_ENTRIES + TUN_RX_ENTRIES + i) as u16);
        }
    ...
    pub fn get_free_bid(&mut self) -> Option<u16> {
        self.free_tx_bids.pop()
    }

    pub fn release_bid(&mut self, bid: u16) {
        self.free_tx_bids.push(bid);
    }
    ...
    pub fn arm_tun_read(&mut self, tun_fd: i32, bid: u16) {
        ...
        let sqe = opcode::Read::new(types::Fd(tun_fd), payload_addr, max_payload)
            .build()
