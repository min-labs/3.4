...
impl fmt::Display for NextNode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            NextNode::Parity => write!(f, "parity"),
        }
    }
}
    ...
    while i + 4 <= n {
        ...
        for j in 0..4 {
            ...
            match disp.next[idx] {
                ...
                NextNode::Parity => { parity_out.push(*desc); }
                NextNode::Consumed => {}
            }
        }
        i += 4;
...
pub struct GraphCtx<'a> {
    ...
    // V1/V3: DRL Action-Space SPSC Bridge
    pub drl_state_tx: Option<&'a mut crate::engine::spsc::Producer<crate::engine::drl::DrlStateTensor>>,
    // V1/V5: DO-178C Watchdog Read Target
    pub target_pacing_ptr: *const [crate::engine::drl::DrlPacingState; 256],
}
