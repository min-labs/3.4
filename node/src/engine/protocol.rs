...
pub const HANDSHAKE_RETX_INTERVAL_NS: u64 = 2_000_000_000;
...
impl Assembler {
    ...
    #[inline(always)]
    pub fn feed<F>(
        &mut self, msg_id: u16, index: u8, total: u8, offset: u16, flags: u8,
...
pub fn send_fragmented_udp<F>(
    ...
    for p in 0..parity_count {
        let p_idx = total + p;
        let flen = ETH_HDR_SIZE + M13_HDR_SIZE + FRAG_HDR_SIZE + p_len;
        frame[0..6].copy_from_slice(dst_mac);
        frame[6..12].copy_from_slice(src_mac);
        frame[12] = (ETH_P_M13 >> 8) as u8;
        frame[13] = (ETH_P_M13 & 0xFF) as u8;
        ...
        let fh = ETH_HDR_SIZE + M13_HDR_SIZE;
        frame[fh..fh+2].copy_from_slice(&msg_id.to_le_bytes());
        frame[fh+2] = p_idx as u8; frame[fh+3] = total as u8;
...
mod tests {
    ...
    fn single_fragment_completes() {
        ...
        asm.feed(1, 0, 1, 0, 0, b"hello", 100, |data| result.extend_from_slice(data));
