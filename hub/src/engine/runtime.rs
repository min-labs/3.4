...
impl HexdumpState {
    ...
    /// # Safety
    /// `frame` must point to a valid readable buffer of at least `len` bytes.
    pub unsafe fn dump_tx(&mut self, frame: *const u8, len: usize, now_ns: u64) {
        if !self.enabled { return; }
        if now_ns.saturating_sub(self.last_tx_ns) < HEXDUMP_INTERVAL_NS { return; }
        self.last_tx_ns = now_ns;
        // SAFETY: frame pointer is from UMEM or kernel ring, validated by caller.
        unsafe { dump_frame("[HUB-TX]", frame, len); }
    }
}
...
/// # Safety
/// `frame` must point to a valid readable buffer of at least `len` bytes.
pub unsafe fn dump_frame(label: &str, frame: *const u8, len: usize) {
