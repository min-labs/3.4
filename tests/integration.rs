...
fn make_test_ctx<'a>(
    ...
    GraphCtx {
        ...
        pacer: None,
        drl_state_tx: None,
        target_pacing_ptr: std::ptr::null(),
    }
}
...
fn pipeline_valid_tunnel_packet_reaches_tun_write() { run_with_large_stack(|| {
    ...
    datapath::classify_route(&all_packets, &mut disp, &mut ctx, &mut stats, false);
    ...
    let mut parity_out = PacketVector::new();
    scatter(
        &all_packets, &disp,
...
fn classify_routes_all_flag_types_correctly() {
    ...
    for &(flags, expected_next, label) in test_cases {
        ...
        datapath::classify_route(&pv, &mut disp, &mut ctx, &mut stats, false);
...
fn scatter_distributes_to_correct_bins() {
    ...
    let mut parity = PacketVector::new();
    scatter(
        &pv, &disp,
...
fn encrypted_pipeline_correct_key() { run_with_large_stack(|| {
    ...
    datapath::classify_route(&decrypt_vec, &mut classify_disp, &mut ctx, &mut stats, false);
...
fn encrypted_pipeline_multi_peer_concurrent() { run_with_large_stack(|| {
    ...
    for (i, key_bytes) in key_material.iter().enumerate().take(NUM_PEERS) {
        let key = LessSafeKey::new(UnboundKey::new(&AES_256_GCM, key_bytes).unwrap());
    ...
    for (i, expected_payload) in original_payloads.iter().enumerate().take(NUM_PEERS) {
        ...
        assert_eq!(dec, &expected_payload[..],
...
fn fragment_reassembly_three_fragments() {
    ...
    assembler.feed(1, 0, 3, 0, 0, &original_data[0..100], 1_000_000_000,
    ...
    assembler.feed(1, 2, 3, 200, 0, &original_data[200..300], 1_000_000_001,
        |data| reassembled.extend_from_slice(data));
    assert!(reassembled.is_empty(), "Still incomplete after 2/3 fragments");

    // Fragment 1 (final, out of order): bytes [100..200]
    assembler.feed(1, 1, 3, 100, 0, &original_data[100..200], 1_000_000_002,
        |data| reassembled.extend_from_slice(data));
...
fn fin_flag_graceful_close() {
    ...
    let mut disp = Disposition::new();
    let mut stats = CycleStats::default();
    datapath::classify_route(&vec, &mut disp, &mut ctx, &mut stats, false);
    ...
    datapath::classify_route(&vec, &mut disp2, &mut ctx, &mut stats2, true);
    assert!(stats2.fin_events[0].1,
