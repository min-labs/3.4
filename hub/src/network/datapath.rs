...
fn decrypt_one(
    ...
    if crate::cryptography::aead::open_frame(frame, cipher, DIR_HUB_TO_NODE, m13_off) {
        ...
        if ctx.peers.slots[pidx].frame_count >= REKEY_FRAME_LIMIT
...
pub fn classify_route(
    ...
    while i + 4 <= n {
        ...
        for j in 0..4 {
            disp.next[i + j] = classify_one(&input.descs[i + j], ctx, stats, closing);
        }
        i += 4;
    }
    while i < n {
        disp.next[i] = classify_one(&input.descs[i], ctx, stats, closing);
        i += 1;
    }
}

#[inline(always)]
fn classify_one(desc: &PacketDesc, ctx: &mut GraphCtx<'_>, stats: &mut CycleStats, closing: bool) -> NextNode {
    let flags = desc.flags;

    // V3: Zero-ALU CIF Telemetry Harvest
    // V3/V4: Zero-ALU CIF Telemetry Harvest & State Tensor Population
    if desc.m13_offset > 0 && flags & crate::engine::protocol::FLAG_TUNNEL != 0 && desc.peer_idx < crate::engine::protocol::MAX_PEERS as u8 {
        let pidx = desc.peer_idx as usize;
        // SAFETY: Pointer math bounds validated via kernel descriptor limits.
        let m13_ptr = unsafe { ctx.umem_base.add(desc.addr as usize + desc.m13_offset as usize) };
        let rx_loss_q8 = unsafe { *m13_ptr.add(45) };
        let burst_q8 = unsafe { *m13_ptr.add(46) };
        let tx_queue_pressure = ctx.scheduler.pending() as u16;

        // HARVEST GROUND-TRUTH PHYSICS (Zero-cost read-and-clear via std::mem::replace)
        let rx_jitter_us = (ctx.peers.jitter_bufs[pidx].estimator.get() / 1000) as u32;
        let recovered_pkts = std::mem::replace(&mut ctx.peers.slots[pidx].drl_recovered_pkts, 0);
        let tx_drops = std::mem::replace(&mut ctx.peers.slots[pidx].drl_tx_drops, 0);

        // Push to SPSC if channel degrades, queue builds, OR positive reinforcement exists
        if rx_loss_q8 > 0 || burst_q8 > 0 || tx_queue_pressure > 500 || recovered_pkts > 0 {
            if let Some(ref mut drl_tx) = ctx.drl_state_tx {
                let state = crate::engine::drl::DrlStateTensor {
                    peer_idx: desc.peer_idx as u16,
                    tx_queue_pressure,
                    rx_loss_q8,
                    burst_q8,
                    _pad0: 0,
                ...
                let _ = drl_tx.push_batch(&[state]); // Lock-free push
            }
        }
    }
    ...
    if flags & FLAG_FEC != 0 { return NextNode::Parity; }
...
pub fn tun_read_batch(
    ...
    if let Some(ref mut free_cons) = ctx.free_to_dp_cons {
        ...
        for &idx in free_buf.iter().take(freed) {
            ctx.slab.free(idx);
        }
    }
    ...
    if let Some(ref mut free_prod) = ctx.free_to_tun_prod {
        ...
        if provision_cap > 0 {
            ...
            if slab_count > 0 {
                ...
                for &idx in slab_buf.iter().take(slab_count).skip(pushed) {
                    ctx.slab.free(idx);
                }
            }
        }
    ...
    if let Some(ref mut cons) = ctx.rx_tun_cons {
        ...
        for desc in desc_buf.iter().take(popped) {
            output.push(*desc);
        }
        return popped;
    }
...
pub fn teardown_all_hub(if_name: &str) {
    use std::process::Command;
    eprintln!("[M13-TEARDOWN] Tearing down all Hub state...");
    ...
    eprintln!("[M13-TEARDOWN] ✓ All Hub state destroyed.");
}
