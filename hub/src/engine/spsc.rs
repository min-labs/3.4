...
impl<T: Copy> Producer<T> {
    ...
    pub fn push_batch(&mut self, items: &[T]) -> usize {
        ...
        #[allow(clippy::needless_range_loop)] // Loop indexes both items[i] and ring via pointer arithmetic
        for i in 0..to_push {
...
impl<T: Copy> Consumer<T> {
    ...
    pub fn pop_batch(&mut self, out: &mut [T]) -> usize {
        ...
        #[allow(clippy::needless_range_loop)] // Loop indexes both out[i] and ring via pointer arithmetic
        for i in 0..to_pop {
            // SAFETY: index is masked to buffer bounds; buffer is valid for capacity elements.
